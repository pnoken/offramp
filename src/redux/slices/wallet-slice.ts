...
const initialTokenBalances: TokenBalance[] = [
  ...
  },
  {
    ...
    amount: 500.0,
    usdRate: 1,
    image: `/images/currencies/usd.png`,
  },
  {
    token: "fiatNGN",
    amount: 10000.0,
    usdRate: 0.0006,
    image: `/images/currencies/ngn.png`,
  },
  {
    token: "fiatGBP",
    amount: 300.0,
    usdRate: 1.315,
    image: `/images/currencies/gbp.png`,
  },
  {
    token: "fiatEUR",
    amount: 350.0,
    usdRate: 1.11,
    image: `/images/currencies/eur.png`,
  },
];
...
const walletSlice = createSlice({
  ...
  reducers: {
    ...
    verifyCredential: (state, action: PayloadAction<{ issuer: string }>) => {
      const credential = state.customerCredentials.find(
        (vc) => vc.issuer === action.payload.issuer
      );
      if (credential) {
    ...
    updateTokenBalance: (state, action: PayloadAction<TokenBalance>) => {
      const index = state.tokenBalances.findIndex(
        (tb) => tb.token === action.payload.token
      );
      if (index !== -1) {
    ...
    setMasterPassword: (
      state,
      action: PayloadAction<{ encryptedPassword: string; iv: string }>
    ) => {
    ...
    updateBalanceAfterExchange: (
      state,
      action: PayloadAction<BalanceUpdatePayload>
    ) => {
      ...
      const fromBalanceIndex = state.tokenBalances.findIndex(
        (tb) => tb.token === fromCurrency
      );
      ...
      const toBalanceIndex = state.tokenBalances.findIndex(
        (tb) => tb.token === toCurrency
      );
      if (toBalanceIndex !== -1) {
        state.tokenBalances[toBalanceIndex].amount += Number(
        ...
        state.tokenBalances.push({
          token: toCurrency,
          amount: Number(finalAmountAfterDeduction),
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(initializeWallet.fulfilled, (state, action) => {
        if (action.payload) {
          (state.customerDid = action.payload), (state.walletCreated = true);
        }
      })
