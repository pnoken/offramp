...
const OfferingDetails: React.FC<OfferingDetailsProps> = ({ onBack }) => {
  const dispatch = useAppDispatch();
  const router = useRouter();
  const { exchanges, isFetching, error } = useAppSelector(
  ...
  const mostRecentExchange = exchanges[
    exchanges.length - 1
  ] as ExchangeWithPfiDid;
  ...
  const fee = (Number(payoutAmount) * 0.003).toFixed(2);
    ...
    >
      <div className="max-w-4xl mx-auto">
        ...
        >
            ...
            <div className="flex justify-between items-center bg-white/10 p-4 rounded-xl">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-white">
                    {Number(payoutAmount).toFixed(2)} {payoutCurrency}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white/10 p-4 rounded-xl mb-8">
            <p className="text-sm text-white/80 mb-2">Fee Information</p>
            <p className="text-lg font-semibold text-white">
              A fee of {fee} {payoutCurrency} will be deducted from your
              balance.
            </p>
          </div>
