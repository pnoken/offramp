...
import { createExchange } from "@/redux/slices/exchange-slice";
...
export const SwapSection: React.FC<{
  ...
  const selectedBalance =
    tokenBalances.find((token) => token.token === selectedCurrencyPair.from)
      ?.amount || 0;
  ...
  const filteredFromCurrencies = currencies.filter(
    (currency) => currency !== selectedCurrencyPair.to
  );
  const filteredToCurrencies = currencies.filter(
    (currency) => currency !== selectedCurrencyPair.from
  );
  ...
  return (
    <>
      {showOfferingDetails ? (
        <OfferingDetails onBack={() => setShowOfferingDetails(false)} />
      ) : (
        <motion.div
